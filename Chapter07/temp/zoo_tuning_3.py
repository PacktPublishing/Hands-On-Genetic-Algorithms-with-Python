...
class ZooTuning:
    ...
    def getAccuracyScore(self, params):
        ...
        #kernel_value = 'linear' if params[2] > 0.5 else 'rbf'
        kernel_value = ['linear', 'rbf'][round(params[2])]
        #print(params[2], "->", kernel_value)
        self.classifier = SVC(random_state=self.randomSeed, C=C_value, gamma=gamma_value, kernel=kernel_value)
