...
def zooClassificationAccuracy(individual):
    ...
    if numFeaturesUsed == 0:
        ...
        accuracy = zoo.getAccuracyScore(individual)
...
def main():
    ...
    for i in range(HALL_OF_FAME_SIZE):
        print(i, ": ", hof.items[i], ", fitness = ", hof.items[i].fitness.values[0],
              ", accuracy = ", zoo.getAccuracyScore(hof.items[i]), ", features = ", sum(hof.items[i]))

    allOnes = [1] * len(zoo)
    print("all features: ", allOnes, ", accuracy = ", zoo.getAccuracyScore(allOnes))
